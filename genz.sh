...
# // Check if the IP is registered in Database
ALLOWED_IPS_URL="https://raw.githubusercontent.com/Amchapeey/strategic/main/Database"

# Debug: Fetch and display allowed IPs
echo -e "${OK} Fetching allowed IPs from: $ALLOWED_IPS_URL"
ALLOWED_IPS=$(curl -s "$ALLOWED_IPS_URL")
echo -e "${OK} Allowed IPs:\n$ALLOWED_IPS"

# Check if the IP exists in the allowed IPs
if echo "$ALLOWED_IPS" | grep -Ev '^###' | grep -q "$IP"; then
