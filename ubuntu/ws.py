...
MSG = '<b><font color="#30e528">WEBSOCKET&nbsp;</font><font color="#ffffffff">BY&nbsp;</font><font color="#30e528">CHAPEEY'
BIG = '<big>'
FTAG = '</big></font>'
NTAG = '</p>'
RESPONSE = f"HTTP/1.1 101 {STYL}{BIG}{MSG}{FTAG}{NTAG}\r\nUpgrade: websocket\r\nConnection: Upgrade\r\nSec-WebSocket-Accept: foo\r\nX-Powered-By: Chapeey\r\n\r\n"

# Load configuration from tun.conf
with open('/usr/bin/tun.conf', 'r') as file:
    config = yaml.safe_load(file)

DEFAULT_HOSTS = [f"{entry['target_host']}:{entry['target_port']}" for entry in config['listen']]
...
class ConnectionHandler(threading.Thread):
    ...
    def run(self):
        try:
            ...
            print(f"Received data from client: {self.client_buffer}")
                ...
                try:
                    ...
                    if data:
                        if in_ is self.target:
                            print(f"Received data from target: {data}")
                            self.client.send(data)
                        else:
                            while data:
                                print(f"Sending data to target: {data}")
